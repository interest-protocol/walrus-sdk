...
(async () => {
  ...
  tx.moveCall({
    ...
    function: 'edit',
    arguments: [
      tx.object(OWNED_OBJECTS.testnet.BLIZZARD_STAKE_NFT_DISPLAY),
      tx.pure.string('image_url'),
      tx.pure.string('https://api.winterwalrus.com/nft/{id}.png'),
    ],
    typeArguments: [TYPES.testnet.BLIZZARD_STAKE_NFT],
  });
  ...
  tx.moveCall({
    ...
    arguments: [tx.object(OWNED_OBJECTS.testnet.BLIZZARD_STAKE_NFT_DISPLAY)],
    typeArguments: [TYPES.testnet.BLIZZARD_STAKE_NFT],
  });
